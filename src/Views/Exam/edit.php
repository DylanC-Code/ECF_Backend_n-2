<section class="p-8">
  ...
  <form action="/exams/edit/<?= $data['id'] ?>" method="POST" class="w-2/5 h-[50vh] mx-auto bg-white px-8 rounded-lg">
    ...
    <fieldset class="flex my-8 p-4 items-center">
      ...
      <select name="matiere" value="<?= $data['matiere'] ?>" class="input rounded-lg">Changer matière
    ...
    <fieldset class="flex my-8 p-4 items-center">
      ...
      <input type="number" name="note" step="0.01" value="<?= $data['note'] ?>" class="input rounded-lg">
    </fieldset>
